# Skill: Fact-Check Methodology (shared base for the `arbitrate` service's proposer role)

This is the base of the system prompt `scripts/services/arbitrate.mjs` sends to whichever model is
proposing in a given pass (read via `readFileSync`, then `arbitrate-propose-appendix.md` is
appended to it — see that file for the one thing added on top). It governs *what the reviewer
checks and how it reports back*. Keep this file in `scripts/services/` next to `arbitrate.mjs`,
which reads it at runtime — that's a different role from `content-authoring-skill.md`/
`json-import-skill.md`, which Claude reads in chat when drafting. This one is never read by
Claude; it's read by whichever model is proposing that pass, inside the GitHub Action.

This file used to be the standalone `proofread` service's entire prompt (that service, and the
Add Task option that created it, are retired — arbitration does everything proofreading did, plus
proposes and arbitrates an actual fix). The methodology below didn't need to change at all when
that happened, which is exactly the point of the "arbitration is proofreading in a different
package" framing: this document *is* the proofreading check, unchanged; only who's now running it,
and what happens after, changed.
...
(output format) are the two places this differs from what was pasted in originally: Part 0
explains exactly what text this service hands the reviewer and why, and Part 2 swaps the plaintext
finding format for JSON, since the app needs to parse and render these, not print them to a
console. (Part 2's JSON shape below is superseded, for `arbitrate`'s purposes, by
`arbitrate-propose-appendix.md`'s `before`/`after` fields \u2014 that file is the authoritative output
format now; Part 2 here is kept as-is rather than edited, since the appendix already documents the
